...
// 계획 : 기능 구현 (앞으로, 뒤로, 움직이다, 멈춘다, 와이퍼가 유리창을 닦는다. ... 동작 -> method), 재원을 정의 (타이어, 와이퍼, 브레이크패달, 악셀패달, 핸들 .... 필드)
// 필드(field), 프로퍼티(property), 속성(attribute)
// 재원이 있고 동작하는 것을 객체라 하고 이런 객체의 설계도를 타입이라 한다.

// 엔진에 기능을 추가 : 소리 = ?? 고급유를 먹은 소리 = 게임 저급한 소리로

// 객체지향 문법은 설계시 시간이 너무 ㅁ오래 걸려서 구현시 단점 => 재사용을 목적으로 프로그래밍 하는 것은 멍청하다 => 플랫폼의 변화가 많아서 막코딩으로 빠르게 구현하는 것이 더 좋다, =>> ???
// 프로그래머는 어떤 코드던 재사용을 전제로 작업 => 습관을 가지면 코든 코드가 객체지향 문법으로 설계가 된다.
