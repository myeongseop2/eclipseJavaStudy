...
//Mysql : dba(고급개발자를 위한 책), Spring : (개념 설명이 잘 되어 있음, 예제가 실습을 할 수 없는 구버전)

//컴파일 : 소스코드(java) -> 바이너리코드(class)
// 자바 : 컴파일시 발견되는 오류(이클립스의 빨간줄) <-> 런타임 시 발견되는 오류(오류를 발견하기 쉽다.)
// 파이썬 (인터프리터언어 -> 소스코드를 바로 실행) : 편집기가 발견하는 오류(파이참의 빨간줄 <-> 런타임 시 발견되는 오류)

public class L16StreamAPIEx2 {
	//{"products":[{"name":"책상","price":60000,"category":4,quantity=55},{"name":"책상","price":60000,"category":4,quantity=55}}
	// 쇼핑몰에서 판매하는 상품 자료 type = Variable Object(VO), Data Transfer Object (DTO)
	// JSON은 무엇인가요? -> JSON을 사용하는 이유! (통신 시 장점 1.문자열, 2.가볍다, 3.	type을 정의가능, 4.data traveling이 용이 ....)
	// key:valye로 되어 있는 가장 인기 있는 자료구조! JavaScriptObjectNotation(JSON) : 자바스크립트 객체 명세서(객체 설명서) => 리터럴 표기법으로 만들어짐 => 문자열 통신으로 사용
	
//	enum Cate{식품, 의류, 가구, 문구류, 침구류} // 각 필드가 고유값을 갖는 상수기 떄문에 수를 사용하는 다른 상수와 섞어 쓰지 않는다.
	interface Cate{ // db에 저장하는 분류가 보통 수로 되어 있기 때문에	
	...
	class ProductDTO{
		...
		int quantity = 10;
		// 생성자로 필드를 초기화 : 생성자는 new 연산자로 호출하면 객체를 반환하는 함수다.
...
//			.forEach(System.out::println);
		...
		//카테고리가 가구인 것의 가격의 합을 구하세여! (reduce)
...
//			.sum(); // mapToInt일때 사용 가능
			.reduce((p1, p2)->p1+p2);
//			.forEach(System.out::println);
		
		System.out.println("가구의 가격 총합 : " + sumOpt.get());
		
		// 내부반복물을 사용하면  stream 자료를 query 처럼 제어할 수 있다.
...
//		System.out.println(i+10); // 오류 null.intValue()
		...
		i_opt=Optional.of(99); // Optional data를 참조하는 방법
...
//		});
		...
		//////////////////////////////////stream partitioning By true or false 의 2 그룹으로 나누는 것
